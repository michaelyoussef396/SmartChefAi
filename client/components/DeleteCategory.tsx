...
export default function DeleteCategory() {
  ...
  const handleDelete = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:5555/categories/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="max-w-md w-full mx-auto rounded-none md:rounded-2xl p-4 md:p-6 shadow-lg bg-white/80 backdrop-blur-sm">
      <h2 className="font-bold text-2xl text-neutral-800 mb-2">Delete Category</h2>
      <p className="text-neutral-600 text-sm mb-4">
        Are you sure you want to delete this category? Type <strong>delete</strong> below to confirm.
      </p>
